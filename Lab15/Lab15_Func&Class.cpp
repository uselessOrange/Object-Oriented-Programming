...
class BattleHistory{
    public:
        int TotalHeroWins;
        int BattleOutcome[10];
        int PowerDifference[10];
        int iterator;
...
/*
            for (int i = 0; i < 10; i++)
            {
                TotalHeroWins=+BattleOutcome[i];
            }
*/            
            TotalHeroWins=0;
         
            BattleOutcome[10]= { 0 };
            std::fill(BattleOutcome, BattleOutcome + 10, 0);
            PowerDifference[10]= { 0 };
            std::fill(PowerDifference, PowerDifference + 10, 0);
        }

        void updateBattleHistroy(int outcome){
            if (outcome>=0)
            {
                BattleOutcome[iterator]=1;
                PowerDifference[iterator]=outcome;
                TotalHeroWins++;
            }
            else if (outcome<0)
            {
                BattleOutcome[iterator]=0;
                PowerDifference[iterator]=outcome;
            }
            iterator++;
        ...
        void showBattleHistory(){
            std::cout<<"Total Hero Wins: "<<TotalHeroWins<<std::endl;
            std::cout<<"Battle Outcomes: "<<std::endl;
            for (int i = 0; i < 10; i++)
            {
                std::cout<<BattleOutcome[i]<<" ";
            }

            std::cout<<std::endl;
            
            std::cout<<"Power Difference each battle: "<<std::endl;
            
            for (int i = 0; i < 10; i++)
            {
                std::cout<<PowerDifference[i]<<" ";
            }
...
class Battle {
public:
    void operator()(Character& monster, Character& hero,BattleHistory& battle_history) {
        ...
        if (outcome >= 0) {
            ...
            battle_history.updateBattleHistroy(outcome);
